...
for(virus in c("EBOV", "H1N1", "H5N1", "LASV", "MARV", "NiV",  "RSV",  "RVFV", "SFSV")){
  ...
  dt.all <- c()
  snp.all <- c()
  for(x in mixedsort(unique(sub("_\\d+$", "", names(vcf.list)[grep(virus, names(vcf.list))])))){
    genome_name <- sub("_.*", "", x)
    ...
    for(name in sample_names){
      ...
      vcf.df$INDEL <- ifelse(is.indel(vcf.list[[name]]), "INDEL", "SNP")
      colnames(vcf.df)[1:2] <- c("chromosome", "start")
      vcf.df$start <- as.numeric(vcf.df$start)
      vcf.df$end <- vcf.df$start
      vcf.df <- vcf.df[,c("chromosome", "start", "end", "AF", "INDEL")]
      vcf.df <- vcf.df[vcf.df$AF>0.01,]
      for(mut.type in c("SNP", "INDEL")){
        if(nrow(vcf.df[vcf.df$INDEL==mut.type,]) > 0){
          ...
          snp_track <- DataTrack(vcf.df[vcf.df$INDEL==mut.type,], name = paste("SNP frequency", x, sep = "\n"), genome = genome_name, 
                                 groups = factor(paste(name, mut.type, sep = ":"), levels = as.vector(outer(sample_names, c("SNP", "INDEL"), paste, sep=":"))), 
  ...
  pdf(paste0(out.dir, "/", genome_name, "_SNP_filterAF.pdf"), width = 20, height = 15)
  print(xyplot(1 ~ chromosome | chromosome, data = chroms, panel = function(x) {
...
pdf(paste0(outdir, "/", genome_name, "_Coverage.pdf"), width = 15, height = 10)
print(xyplot(1 ~ chromosome | chromosome, data = chroms, panel = function(x) {
